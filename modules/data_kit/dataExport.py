...
class DataExport:
    ...
    def _export_to_excel(self):
        if not self.export_data or (
            len(self.export_data) == 1 and self.export_data[0].shape[0] == 0
        ):
            self.logger.info(
                "==> Data empty, Rows of Data = 0 => Not Export Empty file"
            )
            self._is_successful = True
            return

        if len(self.export_data) > 1 and (
            not self.export_name or len(self.export_data) != len(self.export_name)
        ):
            self.logger.info(
                "==> Config incorrect - Sheet Array None or not map Data => Not Export Empty file"
            )
            return
        ...
        try:
            ...
            with pd.ExcelWriter(
                ...
                for key in self._export_name:
                    ...
                    if df is None:
                        self.logger.info(
                            "==> Sheet Name [{key}] not found in data dictionary."
                        )
                        continue
                    if df.shape[0] > 0 and df.shape[1] > 0:
                        ...
                        if not sheet_name:
                            self.logger.info(
                                "==> Sheet name for key [{key}] is None, skipping sheet."
                            )
                            continue
                        ...
                        if is_header:
                            ...
                            for col_num, value in enumerate(df.columns):
                                worksheet.write(
                                    start_row, col_num, value, header_format
                                )
                        # end header format
                        ...
                        if is_format:
                            ...
                            for index, row in df.iterrows():
                                ...
                                date_format = (
                                    even_date_format
                                    if index % 2 == 0
                                    else odd_date_format
                                )
                                datetime_format = (
                                    even_datetime_format
                                    if index % 2 == 0
                                    else odd_datetime_format
                                )
                                for col_num, cell_data in enumerate(row):
                                    if pd.api.types.is_datetime64_any_dtype(
                                        df.iloc[:, col_num]
                                    ):  # Kiểm tra cột có phải là datetime
                                        format_to_apply = (
                                            datetime_format
                                            if df.iloc[:, col_num].dtype
                                            == "datetime64[ns]"
                                            else date_format
                                        )
                                        worksheet.write(
                        ...
                        self.logger.info(
                            f"==> Sheet Name [{key}]: Data empty, Shape of Data => {df.shape[0]},{df.shape[1]} => Not create this sheet"
                        )
            self._is_successful = True
        except Exception as e:
            self._is_successful = False
    ...
    def _export_to_csv(self):
        if not self.export_data or (
            ...
            self._note += "==> Data empty, Rows of Data = 0 => Not Export Empty file"
            self._is_successful = True
            return
        ...
        if len(self.export_data) > 1 and (
            ...
            self.logger.info(
                "==> Config incorrect - Array None or not map Data => Not Export Empty file"
            )
            return
        ...
        try:
            ...
            for i, df in enumerate(self.export_data):
                ...
                file_name = (
                    f"{self.file_name}{num}_{self.export_name[i]}.{self.file_type}"
                )
                ...
                if df is not None and df.shape[0] > 0 and df.shape[1] > 0:
                    ...
                    self.logger.info(
                        f"==> Data [{i}] {self.export_name[i]} => Data counter: {df.shape[0]}"
                    )
                else:
                    self.logger.info(
                        f"==> Data [{i}] {self.export_name[i]}: Data empty, Shape of Data => {df.shape[0]},{df.shape[1]} => Not create this file"
                    )
            self._is_successful = True
        except Exception as e:
            self._is_successful = False
            self._note += f"==> File Name {file_name} [{self.__class__.__name__}][{inspect.currentframe().f_code.co_name}] Process with Error"
            self.logger.exception(
                f"==> File Name {file_name} Process with Error |Class Writer -> _export_to_csv Error, {e}"
            )
    ...
    def _export_to_any_file(self):
        ...
        try:
            ...
            for i, df in enumerate(self.export_data):
                ...
                file_name = (
                    f"{self.file_name}{num}_{self.export_data[i]}.{self.file_type}"
                )
                if df is not None and df.shape[0] > 0 and df.shape[1] > 0:
                    self._data_frame_export(df, self.file_type, file_name)
                    self.logger.info(
                        f"==> Data [{i}] {self.export_data[i]} => Data counter: {df.shape[0]}"
                    )
                else:
                    self.logger.info(
                        f"==> Data [{i}] {self.export_data[i]}: Data empty, Shape of Data => {df.shape[0]},{df.shape[1]} => Not create this file"
                    )
            self._is_successful = True
        except Exception as e:
            self._is_successful = False
    ...
    def _data_frame_export(self, data, file_format, file_path):
        export_functions = {
            DataConstant.RPT_CSV_FILE_TYPE: lambda: data.to_csv(
                file_path, sep=self._sep, index=False, header=True, encoding="utf-8"
            ),
            DataConstant.RPT_TEXT_FILE_TYPE: lambda: data.to_csv(
                file_path, sep=self._sep, index=False, header=False, encoding="utf-8"
            ),
            DataConstant.RPT_EXCEL_FILE_TYPE: lambda: data.to_excel(file_path),
            DataConstant.RPT_JSON_FILE_TYPE: lambda: data.to_json(
                file_path, orient="records", lines=True
            ),
        }
        export_functions.get(
            file_format.lower(),
            lambda: data.to_csv(
                file_path, sep=self._sep, index=False, header=False, encoding="utf-8"
            ),

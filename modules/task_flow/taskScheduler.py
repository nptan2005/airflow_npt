...
class TaskScheduler:
    ...
    def _report_data(self,result_task:Union[List[TaskCompleted], None],total_duration:pd.Timedelta) -> List[pd.DataFrame]:
        ...
        if result_task and task_counter > 0:
            for item in result_task:
                if item.is_frequency_task:
                    if not item.is_notify_fail and not item.is_finished:
                        continue
                    if not item.is_notify_sucess and not item.is_notify_sucess:
                        continue
                seq += 1
                note = ''
                if item.is_error:
        ...
        if seq > 0:
            ...
            total_tasks = len(df)
            total_completed_tasks = df[df['IsCompleted'] is True].shape[0]

...
class TaskDefinitionModel(BaseModel):
    ...
    def _parse_email_str(
        cls, v: Optional[str]
    ) -> str:  # Keep original for model, parse into separate field
        return v or ""
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        if self.email:
            try:
                ...
                self.parsed_email_list = []
        if self.output_name:
            self.parsed_output_name_list = [
                o.strip()
                for o in self.output_name.replace(";", ",").split(",")
                if o.strip()
            ]
    ...
    class Config:
        ...
        extra = "ignore"
        from_attributes = (
            True  # Allows creating from ORM objects or dicts with matching names
        )
...
def fetch_task_definitions_from_db(
    ...
    sql_call = (
        "SELECT * FROM airflow.get_tasks_for_dag(p_run_time => %s, p_task_id => %s);"
    )

    log.info(
        f"Executing SQL to fetch tasks: {sql_call} with params ({current_run_time}, {task_id_filter})"
    )
    ...
    try:
        ...
        records: List[Tuple] = pg_hook.get_records(
            sql=sql_call, parameters=(current_run_time, task_id_filter)
        )
        ...
        if records:
            ...
            for record_tuple in records:
                ...
                try:
                    ...
                    log.error(
                        f"Pydantic validation error for record {record_dict.get('task_id', 'N/A')}: {e}"
                    )
                except Exception as e_gen:
                    log.error(
                        f"General error parsing record {record_dict.get('task_id', 'N/A')}: {e_gen}"
                    )
        else:
...
current_dag_run_time_slot = int(
    pendulum.now("UTC").strftime("%H%M")
)  # Example: 1607 for 4:07 PM UTC
...
with DAG(
    ...
    airflow_tasks: Dict[
        int, Any
    ] = {}  # Stores Airflow operator instances, keyed by original task_id
    ...
    if not task_objects:
        ...
        for task_def in task_objects:
            ...
            if task_def.task_time_out and task_def.task_time_out > 0:
                operator_args["execution_timeout"] = timedelta(
                    seconds=task_def.task_time_out
                )
            ...
            if task_def.task_type == "BASH":
                ...
                def _execute_python_callable(script_to_execute, **kwargs):
                    ...
                    log_py.info(
                        f"Executing Python task via callable: {kwargs['ti'].task_id}"
                    )
                    ...
                    try:
                        ...
                        exec(
                            script_to_execute, {"log": log_py, "kwargs": kwargs}
                        )  # Provide log and context
                        log_py.info(f"Python task {kwargs['ti'].task_id} completed.")
                    except Exception as e_py_exec:
                        log_py.error(
                            f"Error in Python callable for {kwargs['ti'].task_id}: {e_py_exec}"
                        )
                        raise
                ...
                task_def.task_type in ("SFTP_PUT", "SFTP_GET")
                and task_def.connection_string
            ):
                ...
                task_def.task_type == "EMAIL"
                and task_def.connection_string
                and task_def.parsed_email_list
            ):
                email_subject = f"Airflow Task Notification: {task_def.task_name} (ID: {task_def.task_id})"
                email_content = (
                    script_content
                    or f"Task {task_def.task_name} (ID: {task_def.task_id}) status update."
                )
                ...
                if task_def.is_notify_fail and not task_def.is_notify_sucess:
                    ...
                    operator_args["trigger_rule"] = (
                        "all_success"  # Default, but explicit
                    )
                ...
                log.warning(
                    f"Unhandled or misconfigured task_type '{task_def.task_type}' for task_id {task_def.task_id}. Creating EmptyOperator."
                )
        ...
        for task_def in task_objects:
            ...
            if not current_airflow_task:
                log.warning(
                    f"Could not find created Airflow task for DB task_id {task_def.task_id} during dependency setup."
                )
                continue
        ...
        if (
            not start_pipeline.downstream_task_ids
            and not end_pipeline.upstream_task_ids
            and airflow_tasks
        ):
            log.error(
                "Critical error in dependency setup: No tasks linked to start_pipeline, but tasks exist."
            )
            ...
            log.warning(
                "No tasks were connected to start_pipeline. Check parent_task_id logic."
            )

        if not end_pipeline.upstream_task_ids and airflow_tasks:
            log.warning(
                "No tasks were connected to end_pipeline. Check leaf node detection."
            )

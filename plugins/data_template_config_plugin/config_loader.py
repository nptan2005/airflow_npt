...
from pydantic import BaseModel, Field, field_validator # Re-using Pydantic models for structure
...
class CoreImportTemplate(BaseModel):
    ...
    def parse_bool_str(cls, v): # Handles potential string 'true'/'false' from various sources
        if isinstance(v, str):
...
class DataTemplateConfigLoader:
    ...
    @staticmethod
    @provide_session
    def get_import_template(template_name: str, session: Session = None) -> Optional[CoreImportTemplate]:
        db_template = session.query(DBImportTemplate).filter_by(template_name=template_name).first()
        if not db_template:
            return None
    ...
    @staticmethod
    @provide_session
    def get_all_import_templates(session: Session = None) -> Dict[str, CoreImportTemplate]:
    ...
    @staticmethod
    @provide_session
    def get_export_template(template_name: str, session: Session = None) -> Optional[CoreExportTemplate]:
        db_template = session.query(DBExportTemplate).filter_by(template_name=template_name).first()
        if not db_template:
            return None
    ...
    @staticmethod
    @provide_session
    def get_all_export_templates(session: Session = None) -> Dict[str, CoreExportTemplate]:

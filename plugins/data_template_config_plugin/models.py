...
class DBImportTemplate(Base):
    ...
    template_name = Column(
        String(255), nullable=False, unique=True, comment="e.g., ATOM_POS_SET_TXN"
    )

    # Fields from _ImportTemplate Pydantic model
    header_json = Column(
        JSONB, nullable=True, comment="JSON array of header strings"
    )  # Or Text
    ...
    column_mapping_json = Column(
        JSONB, nullable=True, comment="JSON object for column mappings"
    )  # Or Text

    description = Column(
        Text, nullable=True, comment="Optional description for this template"
    )
...
class DBExportTemplate(Base):
    ...
    template_name = Column(
        ...
        nullable=False,
        unique=True,
        comment="e.g., ACQ_POS_ACCOUNTING_REPORT",
    )
    ...
    sheets = relationship(
        "DBExportSheetConfig",
        back_populates="export_template",
...
class DBExportSheetConfig(Base):
    ...
    export_template_id = Column(
        Integer,
        ForeignKey("task_flow.custom_export_templates.id"),
        nullable=False,
    )

    # This 'sheet_name_key' corresponds to the key in the 'sheets' dictionary of your YAML/Pydantic model
    sheet_name_key = Column(
        String(255),
        nullable=False,
        comment="Key for this sheet, e.g., ACQ_BANK, MERCHANT",
    )
    ...
    sheet_display_name = Column(
        String(255), nullable=False, comment="Actual name of the sheet in Excel"
    )

...
from .models import DBImportTemplate, DBExportTemplate, DBExportSheetConfig
...
class DBImportTemplateView(ModelView):
    ...
    list_columns = ['template_name', 'file_extension', 'table_name', 'procedure_name', 'sftp_conn', 'updated_at']
    show_fieldsets = [
        ('Basic Info', {'fields': ['template_name', 'description', 'file_extension', 'separate']}),
        ('File & SFTP', {'fields': ['sftp_conn', 'sftp_move']}),
        ('Parsing Rules', {'fields': ['start_row', 'end_row', 'start_col', 'end_col', 'is_read_to_empty_row']}),
        ('Database Target', {'fields': ['table_name', 'is_truncate_tmp_table', 'procedure_name', 'external_process']}),
    ...
    add_columns = [
        ...
        'file_extension', 'separate', 'sftp_conn', 'sftp_move',
        'start_row', 'end_row', 'start_col', 'end_col', 'is_read_to_empty_row',
        'table_name', 'is_truncate_tmp_table', 'procedure_name', 'external_process',
        'header_json', 'column_mapping_json'
    ]
    ...
    form_widget_args = {
        'header_json': {'rows': 5, 'placeholder': 'Enter JSON array of strings, e.g., ["ColA", "ColB"]'},
        'column_mapping_json': {'rows': 10, 'placeholder': 'Enter JSON object, e.g., {"SourceColA": "TargetColA"}'},
...
class DBExportSheetConfigViewInline(CompactCRUDMixin, ModelView): # Or just ModelView
    ...
    edit_columns = ['sheet_name_key', 'sheet_display_name', 'is_header', 'sheet_title_name', 'is_format', 'column_mapping_json']
    ...
    form_widget_args = {
        'column_mapping_json': {'rows': 5, 'placeholder': 'JSON: {"SourceCol": "ExcelHeader"}'},
    }
    label_columns = {
...
class DBExportTemplateView(ModelView):
    ...
    list_columns = ['template_name', 'file_extension', 'sftp_conn', 'sftp_move', 'updated_at']
    
    add_columns = ['template_name', 'description', 'file_extension', 'separate', 'sftp_conn', 'sftp_move']
    edit_columns = add_columns
    ...
    show_template = "appbuilder/general/model/show_cascade.html" # Default, but explicit

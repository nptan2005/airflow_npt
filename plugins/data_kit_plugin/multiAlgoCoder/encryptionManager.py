...
from data_kit_plugin.multiAlgoCoder.decoder import decoder
...
class EncryptionManager(BaseModel, _SingletonBase):
    ...
    """
    data_folder: str = Field(default=os.path.join(os.getenv('BIN_PATH', "./bin"), "data"))
    iv_folder: str = Field(default=os.path.join(os.getenv('BIN_PATH', "./bin"), "iv"))
    master_key_file:str = Field(default=os.path.join(os.getenv('BIN_PATH', "./bin"), "key","master_key_ase.bin"))
    master_key:bytes = Field(default_factory=bytes)
    iv_master_key_file:str = Field(default=os.path.join(os.getenv('BIN_PATH', "./bin"), "iv","master_key_iv_ase.bin"))
    iv_master_key:bytes = Field(default_factory=bytes)
    key_file:str = Field(default=os.path.join(os.getenv('BIN_PATH', "./bin"), "key","key_ase.bin"))
    ...
    def update_encrypted_data(self,key_prefix: str):
        path = os.path.join(os.getenv("BIN_PATH", "./bin"), "data", f"{key_prefix}_encryption_data.bin")
    ...
    def update_iv(self,key_prefix: str):
        path = os.path.join(os.getenv("BIN_PATH", "./bin"), "iv", f"{key_prefix}_iv_key_ase.bin")

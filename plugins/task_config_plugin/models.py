...
class TaskDefinition(Base):
    ...
    # Core Identifiers
    id = Column(Integer, primary_key=True, autoincrement=True) # Corresponds to task_id in Pydantic model
    task_name = Column(String(255), nullable=False, unique=True) # Ensuring task names are unique for easier reference

    # Hierarchy and Ordering
    parent_task_id = Column(Integer, ForeignKey(f'{__tablename__}.id'), nullable=True) # Self-referential for parent task
    ...
    script = Column(Text, nullable=True) # For SQL queries, bash commands, Python script content/path
    config_key_name = Column(String(500), nullable=True) # Reference to other configs if needed

    # Scheduling and Timing (simplified for now, can be expanded)
    run_time = Column(Integer, nullable=True) # Interpretation depends on usage (e.g., specific time like 0800, or duration)
    frequency = Column(String(50), nullable=True) # e.g., DAILY, HOURLY, CRON_EXPRESSION
    ...
    sub_task_max_retry = Column(Integer, default=3) # Max retries for sub-tasks if this is a group
    ...
    src_file_type = Column(String(50), nullable=True) # Changed from 15 to 50 for flexibility
    ...
    active = Column(Boolean, default=True) # Is the task definition active for DAG generation
    ...
    __table_args__ = ({"schema": "airflow"},) # Ensures table is created in airflow schema

...
class TaskDefinitionView(ModelView):
    ...
    list_columns = [
        'task_name', 'task_type', 'parent_task_id', 'task_order', 
        'active', 'connection_string', 'frequency', 'updated_at'
    ]

    # Columns to display in the show/details view (can be more comprehensive)
    show_fieldsets = [
        ('Summary', {'fields': ['task_name', 'task_type', 'active', 'parent_task_id', 'task_order']}),
        ('Execution Details', {'fields': ['connection_string', 'script', 'config_key_name']}),
        ('Scheduling', {'fields': ['run_time', 'frequency', 'day_of_week', 'day_of_month', 'start_date', 'end_date']}),
        ('Timing & Retries', {'fields': ['task_time_out', 'retry_number', 'sub_task_max_retry']}),
        ('File/Data Handling', {'fields': [
            'output_name', 'src_folder_name', 'src_file_name', 'src_file_type',
            'dst_folder_name', 'dst_file_name', 'dst_file_type'
        ]}),
        ('Flags & Notifications', {'fields': ['is_header', 'is_notification', 'is_attachment', 'email']}),
        ('Process Control', {'fields': ['process_num']}),
        ('Audit Info', {'fields': [
            'request_department', 'request_user', 'request_date', 
            'process_user', 'process_date', 'created_at', 'updated_at'
        ]}),
    ]
    ...
    add_fieldsets = [
        ('Basic Info', {
            'fields': ['task_name', 'task_type', 'active', 'parent_task_id', 'task_order']
        }),
        ...
        ('Scheduling Parameters', {
            'fields': ['run_time', 'frequency', 'day_of_week', 'day_of_month', 'start_date', 'end_date']
        }),
        ...
        ('Process & Audit (Optional)', {
            'fields': ['process_num', 'request_department', 'request_user', 'request_date'],
    ...
    # Searchable columns
    search_columns = ['task_name', 'task_type', 'connection_string', 'script', 'frequency', 'email']
    ...
    form_widget_args = {
        ...
        'parent_task_id': {
            ...
            'query_func': lambda: self.datamodel.session.query(TaskDefinition.id, TaskDefinition.task_name).all()

...
class PdToFile:
    ...
    def _export_to_excel(self):
        if not self.export_data or (len(self.export_data) == 1 and self.export_data[0].shape[0] == 0):
            self.logger.info('==> Data empty, Rows of Data = 0 => Not Export Empty file')
            self._is_successful = True
            return
        
        if len(self.export_data) > 1 and (not self.export_name or len(self.export_data) != len(self.export_name)):
            self.logger.info('==> Config incorrect - Sheet Array None or not map Data => Not Export Empty file')
            return
        ...
        try:
            ...
            with pd.ExcelWriter(file_name, engine='xlsxwriter', datetime_format='dd/mm/yyyy hh:mm:ss', date_format='dd/mm/yyyy') as writer:
                for key in self._export_name:
                    ...
                    if df is None:
                        self.logger.info('==> Sheet Name [{key}] not found in data dictionary.')
                        continue
                    if df.shape[0] > 0 and df.shape[1] > 0:
                        sheet_name = self.get_sheet_name(key)
                        if not sheet_name:
                            self.logger.info('==> Sheet name for key [{key}] is None, skipping sheet.')
                            continue
    ...
    def _export_to_csv(self):
        ...
        if len(self.export_data) > 1 and (not self.export_name or len(self.export_data) != len(self.export_name)):
            self.logger.info('==> Config incorrect - Array None or not map Data => Not Export Empty file')
            return
        ...
        try:
            ...
            self._is_successful = False
            self._note += '==> File Name {file_name} [{self.__class__.__name__}][{inspect.currentframe().f_code.co_name}] Process with Error'

...
from .models import DBEmailTemplate 
...
class SmtpConfig(BaseModel):
    ...
    use_tls: bool = Field(default=False) # Thường thì SSL và TLS không dùng cùng lúc, SSL ngầm, TLS tường minh
...
class EmailTemplateSender:
    def __init__(self, airflow_smtp_conn_id: str, logger=None):
        ...
        self.chart_image_path: Optional[str] = None # Đường dẫn đến file ảnh chart (nếu có)
    ...
    def _load_smtp_config(self) -> Optional[SmtpConfig]:
        try:
            ...
            config_data = {
                ...
                "use_ssl": extra_options.get("use_ssl", extra_options.get("ssl", False)), # Kiểm tra cả 'ssl'
                "use_tls": extra_options.get("use_tls", extra_options.get("tls", False)),
            }
            return SmtpConfig(**config_data)
        except Exception as e:
            self.logger.error(f"Lỗi khi tải cấu hình SMTP từ Airflow connection '{self.airflow_smtp_conn_id}': {e}")
            return None

    @provide_session
    def _get_template_from_db(self, template_name: str, session: Session = None) -> Optional[DBEmailTemplate]:
        try:
            template = session.query(DBEmailTemplate).filter_by(template_name=template_name).first()
            if not template:
                self._note += f"Không tìm thấy email template '{template_name}' trong CSDL. "
                self.logger.warning(f"Không tìm thấy email template '{template_name}' trong CSDL.")
            return template
        except Exception as e:
            self._note += f"Lỗi khi truy vấn email template '{template_name}': {e}. "
            self.logger.error(f"Lỗi khi truy vấn email template '{template_name}': {e}", exc_info=True)
            return None
    ...
    def _grid_content(self) -> str:
        if not self.data_frames_for_report: return ''
        grid_html = ''
        for df in self.data_frames_for_report:
            if isinstance(df, pd.DataFrame) and not df.empty:
                grid_html += pretty_html_table.build_table(df, 'green_light') # Hoặc style khác
        return grid_html

    def _image_attachment_html(self, embed_id: str) -> str:
        return f"<br><p><img style='max-width: 800px;' alt='Chart' src='cid:{embed_id}' /></p>" if self.chart_image_path else ''

    def _build_mime_message(self, subject: str, to_recipients: List[EmailStr], html_body_rendered: str, embed_id_for_chart: str) -> MIMEMultipart:
        msg = MIMEMultipart('related') # 'related' cho HTML và ảnh inline
        msg['From'] = self.smtp_config.login if self.smtp_config else "unknown_sender@example.com"
        ...
        if self.chart_image_path:
            try:
                with open(self.chart_image_path, 'rb') as fp:
                    img_data = fp.read()
                img_mime = MIMEImage(img_data, name=os.path.basename(self.chart_image_path))
                img_mime.add_header('Content-ID', f'<{embed_id_for_chart}>')
                img_mime.add_header('Content-Disposition', 'inline', filename=os.path.basename(self.chart_image_path))
                msg.attach(img_mime)
            except Exception as e:
                self._note += f"Lỗi khi đính kèm chart '{self.chart_image_path}': {e}. "
                self.logger.error(f"Lỗi khi đính kèm chart '{self.chart_image_path}': {e}", exc_info=True)
        ...
        for file_path_str in self.attachment_paths:
            try:
                ...
                self.logger.error(f"Lỗi khi đính kèm file '{file_path_str}': {e}", exc_info=True)
    ...
    def send_email_with_template(
        ...
        render_context.setdefault('subject', final_subject) # Đảm bảo subject có trong context
        render_context.setdefault('textMessage', render_context.get('text_message', '')) # Cho textMessage
        ...
        try:
            rendered_html_body = self._render_template(db_template.body_html, render_context)
        except Exception as e:
            self._note += f"Lỗi khi render email template '{template_name}': {e}. "
            self.logger.error(f"Lỗi khi render email template '{template_name}': {e}", exc_info=True)
            return False
        ...
        all_recipients = list(set(to_recipients_list + self.cc_recipients + self.bcc_recipients))
        if not all_recipients:
            self._note = "Không có người nhận nào được chỉ định."
            self.logger.error(self._note)
            return False

        mime_message = self._build_mime_message(final_subject, to_recipients_list, rendered_html_body, chart_embed_id)

        try:
            server: Optional[smtplib.SMTP] = None
            if self.smtp_config.use_ssl:
                context_ssl = ssl.create_default_context()
                server = smtplib.SMTP_SSL(self.smtp_config.host, self.smtp_config.port, context=context_ssl)
            else:
            ...
            server.sendmail(self.smtp_config.login or "fallback_sender@example.com", all_recipients, mime_message.as_string())
            server.quit()

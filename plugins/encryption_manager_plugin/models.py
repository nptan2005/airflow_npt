...
class EncryptedDataRef(Base):
    ...
    tab_name = Column(
        ...
        nullable=False,
        unique=True,
        comment="Human-readable identifier or category for the encrypted data",
    )
    ...
    encoded_tab_reference = Column(
        Text,
        nullable=False,
        comment="Base64 encoded reference from multiAlgoCoder.encode_str",
    )
    description = Column(
        Text, nullable=True, comment="Optional description for this encrypted entry"
    )
